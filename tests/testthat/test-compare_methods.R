...
test_that("compare_methods throws appropriate error", {
  expect_error(
    compare_methods(CO, CO, rv, id_col = sub),
    'The column "CO" is missing from `df`'
  )
})

test_that("confint works (not testing accuracy)", {
  set.seed(1)
  comp_co_w_ci_tmp <- suppressMessages(add_confint(
    comp_co,
    nsim = 100,
    .progress = "none"
  ))
  ...
  expect_equal(
    unname(comp_co_w_ci_tmp$BA_stats_ci$loa.lwr),
    c(-2.20, -0.36),
    tolerance = 1
  )
})
...
test_that("gen_ba_stats_df works without ci", {
  ...
  expect_equal(nrow(stats_wo_ci), 14)
})
...
test_that("gen_ba_stats_df works with ci", {
  ...
  expect_equal(
    stats_w_ci$ci.upr[stats_w_ci$stat == "loa.upr"],
    3.40,
    tolerance = 1e-2
  )
})
