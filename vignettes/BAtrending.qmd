...
sim_CO <- simulate_repeated_data(
  ...
  between_sub_sd = 0.5, # Between-subject variation in CO
  within_sub_rel_change_sd = 0.3, # Within-subject variation in CO (relative to the mean CO for the subject).
  var_name = "co_true"
)
head(sim_CO)
...
sim_CO <- sim_CO |>
  mutate(
    ...
    ref = simulate_measurement(
      ...
      sub_bias_sd = log(1.05), # between-subject variation of 5% of true CO.
      residual_error_sd = log(1.05), # within-subject variation of 5% of true CO.
    ...
    alt = simulate_measurement(
      ...
      sub_bias_sd = log(1.1), # between-subject variation of 10% of true CO.
      residual_error_sd = log(1.05), # within-subject variation of 5% of true CO.
      relative_errors = TRUE
    )
  )
...
# BA_sim_abs <- add_confint(BA_sim_abs, nsim = 999) # Increase nsim in practice.
...
# BA_sim_log <- add_confint(BA_sim_log, nsim = 999) # Increase nsim in practice.

plot_BA_combine(BA_sim_log, aspect_ratio = 1, keep_log_scale = TRUE)
# aspect_ratio only affects the scatter plot (left panel) for log-transformed data.
```
...
The results from the Bland-Altman analysis on log-transformed measurements (e.g. bias, LoA and Change LoA) can be exponentiated and interpreted on the original scale. The interpretation of these back-transformed results is that they estimate ratios between the measurements instead of absolute differences.
...
A relative bias of `r round(exp(BA_sim_log$BA_stats$bias), 2)` indicates that the alternative method measures `r round((exp(BA_sim_log$BA_stats$bias)-1)*100, 0)`% more than the reference method on average. Relative limits of agreement define the interval in which ratios between future paired measurements (in a random subject) with the same alternative and reference methods are expected to fall into in 95% of cases.
...
Again, we can generate a table of the exponentiated results with

```{r}
BA_table(BA_sim_log, keep_log_scale = FALSE)
```

Change limits of agreement of `r round(exp(BA_sim_log$BA_stats$change.loa), 2)` is interpreted as: if we measure a change in CO with two methods simultaneously, we expext the one change is at most `r round(exp((BA_sim_log$BA_stats$change.loa)-1)*100, 0)`% higher than the other in 95% of cases.

Percentage errors are not meaningful on log-transformed data, but also not needed. A Bland-Altman analysis on log-transformed data inherently gives measures of agreement that are relative to the measured value. 

To compare results from an analysis on log-transformed measurements with an analysis on absolute (non-transformed) measurements, the SD of differences between $log$ measurements ($D_{log} = log(CO_{alt}) - log(CO_{ref})$) is approximately equal to the SD of differences on raw measurements ($D$) relative to the mean of the measurements:

$$
SD(D_{log_e}) \approx \frac{SD(D)}{mean(CO_{alt}, CO_{ref})},
$$

when $SD(D_{log_e}) < \sim 0.3$.

Thus, $1.96 * SD_{total}$ on the log scale is approximately equal to "percentage error".

This is analogous to how the coefficient of variation ($CV = SD(x)/mean(x)$) is approximately equal to $SD(log(x))$.
[^bib]
...
While `BAtrending` conveniently fits a mixed effects model and extracts relevant variables, it is useful to know how to do this manually.
...
To extract these values programatically use:

```{r}
bias <- fixef(mod)[[1]] # Get intercept from model
sd_components <- as.data.frame(VarCorr(mod))[['sdcor']] # Get variance components (SD)
```
...
calc_BA_stats_from_model <- function(m) {
  ...
  sd_components <- as.data.frame(VarCorr(m))[['sdcor']] # Get variance components (SD)
...
Here we use the simple mean of all measurements. In `compare_methods()` the mean value is estimated from a mixed model that accounts for repeated measurments within subjects (`avg ~ 1 + (1|sub)`). This model is also used for describing the distribution of measurements (e.g. `BA_CO$distribution_stats`).

## Bootstrap confidence intervals

To calculate confidence intervals (CI) for the statistics, we can use a parametric bootstrap on the model.
...
This does not calculate CIs for the percentage errors, since this requires an estimate of the mean CO, which is not part of the mixed model. In `BAtrending`, CIs for percentage error is calculated by scaling the CI for `sd.total`, treating the mean CO as a known value. Ideally, the uncertainty of mean CO should be included in the CI for percentage error.