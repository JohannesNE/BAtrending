...
BA_table <- function(ba_obj, decimals = 2, decimals_pct = 1) {
  ...
  loa_group_label <- list("Limits of agreement (95%)" = which(ba_table$variable == "loa.upr"))

  tab_footnotes <- list(
    "a" = list(
      i = which(ba_table$variable == "trending.precision"),
      j = 1,
      text = "Trending precision (95%) = 2 * Within subject variation (SD)."
    ),
    "b" = list(
      i = which(ba_table$variable == "change.loa"),
      j = 1,
  ...
  names(ba_table_tt) <- c("", "Estimate [95% CI]")
...
format_est_ci <- function(est, lwr = NULL, upr = NULL, fmt_pct = FALSE, decimals = 2, decimals_pct = 1) {
  ...
  # Argument checks
  if (!is.numeric(est)) stop("`est` must be numeric.", call. = FALSE)
  if (length(est) == 0) stop("`est` cannot be empty.", call. = FALSE)
  if (xor(is.null(lwr), is.null(upr))) {
      stop("If one of `lwr` or `upr` is provided, the other must also be. Or both can be NULL.", call. = FALSE)
  }
