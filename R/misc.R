...
format_est_ci <- function(est, lwr = NULL, upr = NULL, fmt_pct = FALSE, decimals = 2, decimals_pct = 1, exponentiate = FALSE) {
    ...
    mapply(format_single_est_ci, est, lwr_arg, upr_arg, fmt_pct,
      MoreArgs = list(decimals = decimals, decimals_pct = decimals_pct, exponentiate = exponentiate), 
      USE.NAMES = FALSE
    )
  }
  
  # Function that formats a single set of est, lwr and upr. Later vectorized with mapply.
  format_single_est_ci <- function(est, lwr = NULL, upr = NULL, fmt_pct = FALSE, decimals = 2, decimals_pct = 1, exponentiate = FALSE) {
    ...
    if (has_full_ci) {
      ...
      if ( !(is.null(lwr) && is.null(upr)) ) cli::cli_abort("Only one CI limit was supplied.")
